...
#include "lisp_mobility_between_subnet_single_encap.h"
...
void
Simulation3::SetPosition (Ptr<Node> node, Vector position)
{
...
Vector
Simulation3::GetPosition (Ptr<Node> node)
{
	Ptr<MobilityModel> mobility = node->GetObject<MobilityModel> ();
...
void
Simulation3::AdvancePosition (Ptr<Node> node)
{
...
void
Simulation3::InstallMapResolverApplication (Ptr<Node> node,
																						Ipv4Address msAddress, Time start,
																						Time end)
{
...
void
Simulation3::InstallEchoApplication (Ptr<Node> echoServerNode,
...
Simulation3::InstallDhcpServerApplication (Ptr<Node> node, Ipv4Address prefix,
																						Ipv4Mask netmask,
																						Ipv4Address dhcpServerAddr,
																						Ipv4Address min_addr,
																						Ipv4Address max_addr,
																						Ipv4Address gateway, Time start,
																						Time end)
{
...
void
Simulation3::InstallDhcpClientApplication (Ptr<Node> node, uint32_t device,
																						Time start, Time end)
{
...
Simulation3::InstallLispRouter (NodeContainer nodes)
{
	...
	curExeBinPath = SystemPath::Join (curExeBinPathList.begin (),
																		curExeBinPathList.end ());
	NS_LOG_WARN("Current executable binary path:"<<curExeBinPath);
	std::list<std::string> rlocFilePath =
		{ "lisp", "mobility_between_network", rlocsFile };
	std::list<std::string> rlocXmlPath =
		{ "lisp", "mobility_between_network", mapServerInitDbFile };

	std::string rlocFP = SystemPath::Append (
			curExeBinPath,
			SystemPath::Join (rlocFilePath.begin (), rlocFilePath.end ()));
	...
	std::string rlocXmlFP = SystemPath::Append (
			curExeBinPath,
			SystemPath::Join (rlocXmlPath.begin (), rlocXmlPath.end ()));
...
//					or
//			(fiveTuple.sourceAddress == "10.1.7.2"
...
main (int argc, char *argv[])
{
	...
	cmd.AddValue ("dhcp-collect", "Time for which offer collection starts",
								dhcp_collect);
	cmd.AddValue ("wifi-beacon-interval",
								"Time for which offer collection starts", wifiBeaconInterval);
	cmd.AddValue ("map-search-time",
								"Time consumed for EID-RLOC mapping search in map server",
								mappingSearchTime);
	...
	Config::SetDefault ("ns3::Ipv4GlobalRouting::RandomEcmpRouting",
											BooleanValue (true));
	Config::SetDefault ("ns3::DhcpClient::Collect",
											TimeValue (Seconds (dhcp_collect)));
	Config::SetDefault ("ns3::MapServerDdt::SearchTime",
											TimeValue (Seconds (mappingSearchTime)));
...
//  NS_LOG_INFO()
	...
	NodeContainer lispRouters = NodeContainer (c.Get (0), c.Get (3), c.Get (6));
	NodeContainer xTRNodes = NodeContainer (c.Get (0), c.Get (3));
...
//  wifiMac.SetType ("ns3::AdhocWifiMac");
	...
	Ssid ssid = Ssid ("xTR1");
	wifiMac.SetType ("ns3::StaWifiMac", "Ssid", SsidValue (ssid), // need to tell STA the target SSID
...
//  Simulator::Schedule(Seconds(20), &FlyPosition, xTR1.Get(0));
	...
	Time END_T = Seconds (65.5);
	Time ECO_END_T = Seconds (65.5);
...
//																START_T, ECO_END_T); // Discard port (RFC 863)
	...
	p2p.EnableAsciiAll (
			ascii.CreateFileStream ("lisp-mobility-between-subnet-single-encap.tr"));
	wifiPhy.EnableAscii (ascii.CreateFileStream ("lisp-mn-single-encap.tr"), 0,
												1);

	wifiPhy.EnablePcapAll ("lisp-mobility-between-subnet-single-encap-wifi",
													true);
//  wifiPhy.EnablePcap("lisp-mobility-between-subnet-wifi2", m_n0Tap);
	p2p.EnablePcapAll ("lisp-mobility-between-subnet-single-encap");
	MobilityHelper::EnableAsciiAll (
			ascii.CreateFileStream ("lisp-mobility-between-subnet-single-encap.mob"));
