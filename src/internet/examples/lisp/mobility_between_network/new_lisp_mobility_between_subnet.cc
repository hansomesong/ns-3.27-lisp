...
Simulation3::PopulateStaticRoutingTable2 (NodeContainer c)
{
	...
	// No need to set route table for intermediate router node 5
...
//  NS_LOG_INFO()
	...
	NodeContainer lispRouters = NodeContainer (c.Get (1), c.Get (2), c.Get (3), c.Get (6));
	NodeContainer xTR1 = NodeContainer (c.Get (1));
	NodeContainer xTR2 = NodeContainer (c.Get (2));
	NodeContainer xTR3 = NodeContainer (c.Get (3));
...
//			Ipv4InterfaceAddress(Ipv4Address("10.1.1.254"), Ipv4Mask("/0"))); // need to remove this workaround
	ipv4xTR2->AddAddress (
			ifIndex,
			Ipv4InterfaceAddress (Ipv4Address ("10.1.1.253"), Ipv4Mask ("/24")));
...
//  Simulator::Schedule(Seconds(20), &FlyPosition, xTR1.Get(0));
	...
	sim->InstallDhcpServerApplication (c.Get (1),
																		 ...
																		 Ipv4Mask ("/24"),
																			...
																			ap1Interface.GetAddress (0),
																			Seconds (0.0), END_T);
	sim->InstallDhcpServerApplication (c.Get (2),
																		 Ipv4Address ("10.1.1.0"),
																			Ipv4Mask ("/24"),
	...
	sim->InstallXtrApplication (xTR1, mrAddr, msAddr, Seconds (0.0), END_T);
	sim->InstallXtrApplication (xTR2, mrAddr, msAddr, Seconds (22.0), END_T);
	sim->InstallXtrApplication (xTR3, mrAddr, msAddr, Seconds (0.0), END_T);
