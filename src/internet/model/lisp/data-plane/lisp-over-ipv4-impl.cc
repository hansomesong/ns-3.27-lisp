...
namespace ns3
{
	...
	{
		static TypeId tid =
				TypeId ("ns3::LispOverIpv4Impl").SetParent<LispOverIpv4> ().SetGroupName (
						"Lisp").AddConstructor<LispOverIpv4Impl> ();
		return tid;
	}
...
// If false, we drop packet in IP, if true OK.
	void
	LispOverIpv4Impl::LispOutput (Ptr<Packet> packet,
																Ipv4Header const &innerHeader,
																...
																Ptr<Ipv4Route> lispRoute)
	{
		...
		if (destLocator == 0)
			{
				...
				m_statisticsForIpv4->IncOutputPackets ();
				NS_LOG_WARN(
						"No valid destination locator for eid " << innerHeader.GetDestination() << ". Drop!");
				return;
			}
		...
		if (destLocator->GetRlocMetrics ()->GetMtu ()
						...
						> destLocator->GetRlocMetrics ()->GetMtu ())
			{
				...
				m_statisticsForIpv4->IncOutputPackets ();
				NS_LOG_DEBUG(
						"MTU DEST " << destLocator->GetRlocMetrics ()->GetMtu () << " Packet size " << size);
				NS_LOG_ERROR(
						"[LISP_OUTPUT] Drop! MTU check failed for destination RLOC.");
				return;
			}
		 ...
		 */
		srcLocator = SelectSourceRloc (
				static_cast<Address> (innerHeader.GetSource ()), destLocator);
		...
		packet = PrependLispHeader (packet, localMapping, remoteMapping, srcLocator,
																destLocator);
		...
		if (Ipv4Address::IsMatchingType (srcLocator->GetRlocAddress ()))
			{
				...
				udpLength = innerHeader.GetPayloadSize ()
						+ innerHeader.GetSerializedSize ()
						+ UdpHeader ().GetSerializedSize ()
						+ LispHeader ().GetSerializedSize ();
//      udpLength = innerHeader.GetPayloadSize ()+ UdpHeader ().GetSerializedSize () + LispHeader ().GetSerializedSize ();

				//NS_LOG_DEBUG ("Building outer header");
				// outer Ipv4 Header
				outerHeader.SetPayloadSize (udpLength);
				...
				outerHeader.SetSource (
						Ipv4Address::ConvertFrom (srcLocator->GetRlocAddress ()));
				outerHeader.SetDestination (
...
// Packets coming from a possible Rloc to enter the AS
	void
	LispOverIpv4Impl::LispInput (Ptr<Packet> packet,
																Ipv4Header const &outerHeader)
	{
		UdpHeader udpHeader;
		...
		if (packet->GetSize ()
				< (udpHeader.GetSerializedSize () + outerHeader.GetSerializedSize ()))
			{
				NS_LOG_ERROR(
						"[LISP_INPUT] Drop! Packet size smaller that headers size.");
		...
		while (metadataIterator.HasNext ())
			{
				...
				if (!(item.tid.GetName ().compare (Ipv4Header::GetTypeId ().GetName ())))
					{
						isMappingForPacket =
								LispOverIp::m_mapTablesIpv4->IsMapForReceivedPacket (
										packet, lispHeader,
						...
						if (innerIpv4Header.GetProtocol () == UdpL4Protocol::PROT_NUMBER)
							{
								/**
								 * Find a BUG that I spent much time but still cannot figure out why:
								 * To check whether the inner packet is a LISP control plan message,
								 * I peel UDP header then add it again to packet. However, after such a
								 * peel/add operation, UDP lenght changes: 8 bytes less than before
								 * operations. I don't why. The workaround is to do peel operations on a
								 * copy packet
								 */

								// note that at this time packet's outer header, udp, LISP header and
								// inner IP header have been removed.
								...
								if (udpHeader.GetDestinationPort ()
									...
									{
										NS_LOG_DEBUG(
												"UDP on port:"<<unsigned(udpHeader.GetDestinationPort ())<<"->LISP Control Plan Message!");
										isControlPlanMsg = true;
									}
							}
						// If inner IP contains a TCP header, how to process? I think do thing...
						// Update inner IP header, for example, decrease TTL
						innerIpv4Header.SetTtl (outerHeader.GetTtl ());
						Address from = static_cast<Address> (innerIpv4Header.GetSource ());
						Address to =
								static_cast<Address> (innerIpv4Header.GetDestination ());
						...
						if (isMappingForPacket or isControlPlanMsg)
							{
								...
								NS_LOG_DEBUG("Before re-injecting IP packet gain, packet content:"<<*packet);
								ipv4->Receive (m_currentDevice, packet, m_ipProtocol, from, to,
																m_currentPacketType);
								NS_LOG_DEBUG(
										"Re-inject the packet in receive to forward it to: " << innerIpv4Header.GetDestination ());
							}
						else
							{
								// TODO drop and log
								NS_LOG_ERROR(
										"Mapping check failed during local deliver! Attention if this is cause by double encapsulation!");
							}
						return;
					}
				// if inner header is ipv6
				else if (!(item.tid.GetName ().compare (
						Ipv6Header::GetTypeId ().GetName ())))
					{
						m_statisticsForIpv6->IncInputDifAfPackets ();
						isMappingForPacket =
								LispOverIp::m_mapTablesIpv6->IsMapForReceivedPacket (
										packet, lispHeader,
										static_cast<Address> (outerHeader.GetSource ()),
										static_cast<Address> (outerHeader.GetDestination ()));
	...
	LispOverIpv4::MapStatus
	LispOverIpv4Impl::IsMapForEncapsulation (Ipv4Header const &innerHeader,
																						Ptr<MapEntry> &srcMapEntry,
																						Ptr<MapEntry> &destMapEntry,
																						Ipv4Mask mask)
	{
		...
		if (srcMapEntry == 0)
			{
				...
				srcMapEntry = LispOverIp::DatabaseLookup (
						static_cast<Address> (innerHeader.GetSource ()));
				NS_LOG_DEBUG(
		...
		if (mask.IsMatch (innerHeader.GetSource (), innerHeader.GetDestination ()))
			{
				NS_LOG_DEBUG(
						"[MapForEncap] No encap needed. Addresses matches in their mask!");
				return LispOverIpv4::No_Need_Encap;
			}

		if (destMapEntry == 0)
			{
				destMapEntry = LispOverIp::CacheLookup (
		...
		if (destMapEntry == 0)
			{
				...
				sockMsgHdr.SetMapAddresses (
						(int) sockMsgHdr.GetMapAddresses ()
								| static_cast<int> (LispMappingSocket::MAPA_EID));
				sockMsgHdr.SetMapType (
						static_cast<uint8_t> (LispMappingSocket::MAPM_MISS));
				...
				SendNotifyMessage (static_cast<uint8_t> (LispMappingSocket::MAPM_MISS),
														packet, sockMsgHdr, 0);
				return LispOverIpv4::No_Mapping;
			}
	 ...
	 */
	bool
	LispOverIpv4Impl::NeedEncapsulation (Ipv4Header const &ipHeader,
																				Ipv4Mask mask)
	{
		...
		Ptr<MapEntry> eidMapEntry = LispOverIp::DatabaseLookup (
		...
		if (eidMapEntry)
			{
				...
				if (mask.IsMatch (ipHeader.GetSource (), ipHeader.GetDestination ()))
					{
						NS_LOG_DEBUG(
								"Why no mapping entry found for EID:"<<eidMapEntry->GetEidPrefix()->GetEidAddress());
						return false;
					}
				NS_LOG_DEBUG(
						"The found EID-RLOC mapping is: EID("<<eidMapEntry->GetEidPrefix()->GetEidAddress()<<") --> RLOC\n("<<eidMapEntry->GetLocators()->Print()<<")");
				return true;
			}
	 ...
	 */
	bool
	LispOverIpv4Impl::NeedDecapsulation (Ptr<const Packet> packet,
																				Ipv4Header const &ipHeader)
	{
		...
		if (packet->GetSize ()
				< (udpHeader.GetSerializedSize () + ipHeader.GetSerializedSize ()
						+ lispHeader.GetSerializedSize ()))
			{
...
// called at the end of lisp_output
	Ptr<Packet>
	LispOverIpv4Impl::LispEncapsulate (Ptr<Packet> packet, uint16_t udpLength,
																			uint16_t udpSrcPort)
	{
