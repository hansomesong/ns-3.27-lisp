...
namespace ns3
{
	...
	LispOverIp::LispOverIp ()
	{
		 ...
		 */
		NS_LOG_FUNCTION(
				this<<"LispOverIp constructor is called, the m_sockets size is: "<<m_sockets.size());
	...
	void
	LispOverIp::Print (std::ostream &os) const
	{
																...
																Ptr<const Locator> destLocator) const
	{
		...
		if (srcRloc != 0)
			{
				NS_LOG_DEBUG(
						"The selected source RLOC address: "<<srcRloc->GetRlocAddress());
			}
		else
			{
				NS_LOG_ERROR("No valid source RLOC is found!");
				NS_LOG_DEBUG(
						"Currently, the mapping database's content: "<<*m_mapTablesIpv4);
			}
		return srcRloc;
	}
	...
	void
	LispOverIp::SetCacheMissCallback (CacheMissCallback cb)
	{
		m_cacheMissCb = cb;
	}
		...
		while ((packet = socket->RecvFrom (from)))
			{
				...
				if (sockMsgHdr.GetMapType ()
						...
						== static_cast<uint16_t> (LispMappingSocket::MAPM_GET))
					{
						...
						== static_cast<uint16_t> (LispMappingSocket::MAPM_DATABASE_UPDATE))
					{
						/**
						...
						uint8_t messageType =
								static_cast<uint8_t> (LispMappingSocket::MAPM_REGISTER);
...
//	    m_rlocsList.erase(curRlocAddr);
						Address rlocAddr =
								msg->GetLocators ()->GetLocatorByIdx (0)->GetRlocAddress ();
						m_rlocsList.insert (rlocAddr);
						for (std::set<Address>::const_iterator it = m_rlocsList.begin ();
								it != m_rlocsList.end (); ++it)
							{
								if (Ipv4Address::IsMatchingType (*it))
									{
										NS_LOG_DEBUG(
												"RLOC list item: "<<Ipv4Address::ConvertFrom(*it));
									}
								else if (Ipv6Address::IsMatchingType (*it))
									{
										NS_LOG_DEBUG(
												"RLOC list item: "<<Ipv6Address::ConvertFrom(*it));
									}
								else
									{
						...
						LispOverIp::SendNotifyMessage (messageType, packet, mapSockHeader,
																						0);
					}
																	...
																	int flags)
	{
		...
		NS_LOG_DEBUG(
				"Send Messages from data plan to control plan: " << m_sockets.size ()-1<<" sockets.");
																	...
																	Ptr<Locator> destRloc)
	{
		...
		if (localMapEntry->IsUsingVersioning ()
				&& remoteMapEntry->IsUsingVersioning ())
			{
																...
																Ptr<LispOverIp> lispOverIp)
	{
		...
		if (header.GetNBit ())
		...
		else if (header.GetVBit ()) // note: N bit and V bit cannot be set in header
			{
				...
				if (localMapEntry->IsUsingVersioning ()
						&& header.GetDestMapVersion ()
								!= localMapEntry->GetVersionNumber ())
					{
						// TODO Question Why Does OpenLisp mask the version number
						if (IsMapVersionNumberNewer (localMapEntry->GetVersionNumber (),
																					header.GetDestMapVersion ()))
							{
								/*
				...
				else if (remoteMapEntry && remoteMapEntry->IsUsingVersioning ()
						&& header.GetSrcMapVersion ()
								!= remoteMapEntry->GetVersionNumber ())
					{
						if (IsMapVersionNumberNewer (remoteMapEntry->GetVersionNumber (),
																					header.GetSrcMapVersion ()))
							{
								/*
				...
				if (header.GetLBit ())
					{
						...
						if (remoteMapEntry && destRloc
								&& header.GetLSBs () != remoteMapEntry->GetLocsStatusBits ())
							{
	...
	uint16_t
	LispOverIp::GetLispSrcPort (Ptr<const Packet> packet)
	{
		return LISP_DATA_PORT;
	}

	bool
	LispOverIp::IsMapVersionNumberNewer (uint16_t vnum2, uint16_t vnum1)
	{
