...
namespace ns3
{
	...
	public:
		...
		virtual Ptr<Locator>
		DestinationRlocSelection (Ptr<const MapEntry> remoteMapEntry) = 0;

		virtual Ptr<Locator>
		SourceRlocSelection (Address const &srcEid,
													Ptr<const Locator> destLocator) = 0;

		virtual bool
		IsMapForReceivedPacket (Ptr<const Packet> p, const LispHeader &header,
														const Address &srcRloc,
														const Address &destRloc) = 0;

		virtual void
		GetMapEntryList (MapEntryLocation location,
											std::list<Ptr<MapEntry> > &entryList) = 0;
		...
		GetLispOverIp (void);
		void
		SetLispOverIp (Ptr<LispOverIp> lispProtocol);
		 ...
		 */
		virtual Ptr<LispEtrItrApplication>
		GetxTRApp ()=0;
		virtual void
		SetxTRApp (Ptr<LispEtrItrApplication> xTRApp)=0;
...
//  Ptr<LispEtrItrApplication> m_xTRApp;
	...
	std::ostream&
	operator<< (std::ostream &os, MapTables const &mapTable);
