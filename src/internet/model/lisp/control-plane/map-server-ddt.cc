	...
	MapServerDdt::GetTypeId (void)
	{
		static TypeId tid =
				TypeId ("ns3::MapServerDdt").SetParent<MapServer> ().SetGroupName (
						"Lisp").AddConstructor<MapServerDdt> ().AddAttribute (
						"SearchTime",
						...
						if (msg->GetM () == 1)
							{
								...
								MapResolver::ConnectToPeerAddress (locator->GetRlocAddress (),
																										m_peerPort, m_socket);
								/**
								 * Update, 02-06-2018, Qipeng
								 * To support network-based mobility, here we make some modifications:
								 * if the RLOC in MapRegister from xTR2, i.e. 10.1.3.1 and EID prefix is 10.1.7.0
								 * Send Map notify to xTR1
								 */
								// Only contains one RLOC
								Ptr<Locator> regisXtrLoc = msg->GetRecord()->GetLocators()->GetLocatorByIdx(0);
								NS_LOG_DEBUG("The xTR in register process is: "<<Ipv4Address::ConvertFrom(regisXtrLoc->GetRlocAddress()));

								if (Ipv4Address::ConvertFrom(regisXtrLoc->GetRlocAddress()).IsEqual(Ipv4Address("10.1.3.1")))
									{
										if(Ipv4Address::ConvertFrom(msg->GetRecord()->GetEidPrefix()).IsEqual(Ipv4Address("10.1.1.0")))
											{
												NS_LOG_DEBUG("Since the xTR in register process is: "
														<<Ipv4Address::ConvertFrom(regisXtrLoc->GetRlocAddress())
														<<". Send back to xTR1 "<<Ipv4Address("10.1.2.1")
												);
												MapResolver::ConnectToPeerAddress (static_cast<Address>(Ipv4Address("10.1.2.1")), m_peerPort, m_socket);
											}
									}
						...
						== static_cast<uint8_t> (MapRequestMsg::GetMsgType ()))
					{
						...
						if (entry == 0)
						...
						else
							{
								...
								Ptr<Packet> reqPacket = Create<Packet> (buf,
																												packet->GetSize ());
								Simulator::Schedule (m_searchTime, &MapServerDdt::Send, this,
																			reqPacket);
