		...
		while ((packet = socket->RecvFrom (from)))
			{
				...
				if (msg_type == MapRequestMsg::GetMsgType ())
				...
				else if (msg_type == static_cast<uint8_t> (LispControlMsg::MAP_NOTIFY))
					{
						 ...
						 */
						NS_LOG_INFO("Case: reception of map notification");
						Ptr<MapNotifyMsg> notifyMsg = MapNotifyMsg::Deserialize (buf);
						// If the mapping content of MAP_NOTIFY is not present in its database. Add them into database
						Address eidAddr = notifyMsg->GetRecord()->GetEidPrefix();
						Address corrRlocAddr = notifyMsg->GetRecord()->GetLocators()->GetLocatorByIdx(0)->GetRlocAddress();
						...
						if (m_mapTablesV4->GetNMapEntriesLispCache () != 0
								or m_mapTablesV6->GetNMapEntriesLispCache () != 0)
							{
								...
								LispEtrItrApplication::SendSmrMsg (Create<EndpointId> (Ipv4Address("10.1.1.1"), Ipv4Mask ("/32")));
								/**
...
//		tmpEid = (*it)->GetEidPrefix();
	...
	// As a first step, we send SMR to all xTRs in the cache databases.
		void
		LispEtrItrApplication::SendSmrMsg (Ptr<EndpointId> eid)
		{
			// First extract all map entries in m_mapTablesV4 and m_mapTablesV6
			NS_LOG_FUNCTION(this);
			std::list<Ptr<MapEntry> > mapEntries;
