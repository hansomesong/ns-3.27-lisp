...
//				MakeCallback (&LispEtrItrApplication::HandleRead, this));
	...
	void
	LispEtrItrApplication::SendOneMapRegister (Ptr<MapEntry> mapEntry)
	{
		...
		while ((packet = socket->RecvFrom (from)))
			{
				...
				if (msg_type == MapRequestMsg::GetMsgType ())
						...
						if (requestMsg->GetS () == 0)
							{
								...
								if (mapReply != 0)
									...
									{
										NS_LOG_WARN(
												"Negative map reply case! No message will be send to xTR:"<<Ipv4Address::ConvertFrom (requestMsg->GetItrRlocAddrIp()));
									}
						...
						else if (requestMsg->GetS () == 1 and requestMsg->GetS2 () == 0)
							{
								...
								if (m_node->GetObject<LispOverIpv4> ()->IsLocatorInList (
										rlocAddr))
									{
									...
									{
										NS_LOG_DEBUG(
												"The destination IP address: " <<Ipv4Address::ConvertFrom (rlocAddr) <<" is not a known RLOC! In LISP-MN, it maybe an EID...Send Map request to query it");
				...
				else if (msg_type == static_cast<uint8_t> (MapReplyMsg::GetMsgType ()))
					{
						 ...
						 */
						for (std::list<Ptr<MapRequestMsg>>::const_iterator it =
								m_mapReqMsg.begin (); it != m_mapReqMsg.end (); ++it)
							{
								//
							}
				...
				else if (msg_type == static_cast<uint8_t> (LispControlMsg::MAP_NOTIFY))
					{
						...
						if (m_mapTablesV4->GetNMapEntriesLispCache () != 0
								or m_mapTablesV6->GetNMapEntriesLispCache () != 0)
							{
								 ...
								 */
								m_resendSmrEvent = Simulator::Schedule (
										Seconds (2.0), &LispEtrItrApplication::SendSmrMsg, this);
								m_recvIvkSmr = false;
							}
					}
				else
...
//		tmpEid = (*it)->GetEidPrefix();
	...
	void
	LispEtrItrApplication::SendInvokedSmrMsg (Ptr<MapRequestMsg> smr)
	{
		Ptr<Packet> reactedPacket;
		/**
		...
		if (smr->GetItrRlocAddrIp () != static_cast<Address> (Ipv4Address ()))
			{
				MapResolver::ConnectToPeerAddress (smr->GetItrRlocAddrIp (), m_peerPort,
																						m_socket);
				NS_LOG_DEBUG(
						"Now the socket has been binded to:"<<Ipv4Address::ConvertFrom(smr->GetItrRlocAddrIp()));
			}

		else if ((smr->GetItrRlocAddrIpv6 ()
				!= static_cast<Address> (Ipv6Address ())))
		...
		if (Ipv4Address::IsMatchingType (itrAddress))
			{
				...
				NS_LOG_DEBUG(
						"Invoked-SMR's source RLOC field has been set as:"<<Ipv4Address::ConvertFrom(itrAddress));
			}
		else
			{
	...
	void
	LispEtrItrApplication::SendMapRequest (Ptr<MapRequestMsg> mapReqMsg)
	{
	...
	void
	LispEtrItrApplication::SendMapRequest2 (Address eidPrefix)
	{
		/**
		 * TODO: whehter here 64 bytes here is good...
		 ...
		 */
		Ptr<MapRequestMsg> mapReqMsg = GenerateMapRequest (
				Create<EndpointId> (eidPrefix, Ipv4Mask ("/32")));
		uint8_t bufMapReq[64];
		mapReqMsg->Serialize (bufMapReq);
		Ptr<Packet> packetMapReqMsg;
		packetMapReqMsg = Create<Packet> (bufMapReq, 64);
...
//	}
	...
	std::list<Ptr<MapRequestMsg>>
	LispEtrItrApplication::GetMapRequestMsgList ()
	{
	...
	Ptr<MapEntry>
	LispEtrItrApplication::GenerateMapEntry (Ptr<MapReplyMsg> replyMsg)
	{
		Ptr<MapEntry> mapEntry = Create<MapEntryImpl> ();

		Ptr<MapReplyRecord> replyRecord = replyMsg->GetRecord ();

		mapEntry->SetLocators(replyRecord->GetLocators());
		std::stringstream ss;
		...
		if (record->GetAfi () == LispControlMsg::IP)
			{
				...
				NS_LOG_DEBUG(
						"Execute database look up for EID: "<<Ipv4Address::ConvertFrom(record->GetEidPrefix()));
				entry = m_mapTablesV4->DatabaseLookup (record->GetEidPrefix ());
			}
		else if (record->GetAfi () == LispControlMsg::IPV6)
			{
				NS_LOG_DEBUG(
						"Execute database look up for EID: "<<Ipv6Address::ConvertFrom(record->GetEidPrefix()));
	...
	void
	LispEtrItrApplication::SetMapReplyCallback(MapReplyCallback cb)
	{
		m_MapReplyCb = cb;
	}
